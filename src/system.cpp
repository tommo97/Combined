...
void SYSTEM::PutWakesInTree() {
    ...
    for (int i = 0; i < BODY::VortexPositions.size(); ++i)              // set from 0.0275 (Barltrop) to 0.0625 (SOTON)
        if ((BODY::VortexPositions[i] - *BODY::VortexOrigins[i]).Mag() > (BODY::MinDistance[BODY::VortexOwnerID[i] - 1] * SYSTEM::GambitScale)) {
            Num2Insert++;
            toInsert[i] = true;
        } else
            Num2Keep++;
...
#pragma omp parallel for
    for (int i = 0; i < BODY::AllBodyFaces.size(); ++i) {
    BODY::AllBodyFaces[i]->Vfmm1 = BODY::AllBodyFaces[i]->Vfmm0 = UTIL::interp3Pointer <Vect3 > (BODY::AllBodyFaces[i]->Xp, BODY::AllBodyFaces[i]->Vp, BODY::AllBodyFaces[i]->CollocationPoint);
