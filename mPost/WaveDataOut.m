...
for i = 1:size(CpHistory,1)
    ...
    TriMomPos = blade.RADIUS(1:end-1) + 2*diff(blade.RADIUS)/3;
    LinMomPos = blade.RADIUS(1:end-1) + diff(blade.RADIUS)/2;
    LinWt = FoilArea * (rhoSteel - rhoH2O) * 9.80665 * blade.CHORD(1:end-1) .* diff(blade.RADIUS);
    TriWt = FoilArea * (rhoSteel - rhoH2O) * 9.80665 * diff(blade.CHORD) .* diff(blade.RADIUS) / 2;
    
    SelfMoment_(i) = cos(deg2rad(th) + BodyRates0_x*Times(i)) *(sum(LinWt.*LinMomPos) + sum(TriWt.*TriMomPos));
    
    dwdx = diff(blade.CHORD) .* diff(blade.RADIUS);
    w0 = blade.CHORD(1:end-1);
    x0 = blade.RADIUS(1:end-1);
    x1 = blade.RADIUS(2:end);
    
    M0 = x0.*(dwdx.*x0.*x0/3 + w0.*x0/2);
    M1 = x1.*(dwdx.*x1.*x1/3 + w0.*x1/2);
    SelfMoment__(i) = FoilArea * (rhoSteel - rhoH2O) * 9.80665 * cos(deg2rad(th) + BodyRates0_x*Times(i)) * sum(M1 - M0);
